...
function fuzzyMatch(query, list)
    ...
    for _, item in ipairs(list) do
        -- Convert item to lower for comparison
		if string.find(string.lower(item), lowerQuery, 1, true) then
			results = true
        end
    end
    return results
...
function customize_enfeeble_sets(spell)
	if spell.skill == 'Enfeebling Magic' then
		if spell.name ~= 'Dispelga' then 
			equip(sets.macc) -- need to lock weapon and this should already set_combine() with macc
		end	
		if fuzzyMatch(spell.name, enfeebs.skill) then
			equip(sets.midcast.Enfeebling.Skill)
		end
		if fuzzyMatch(spell.name, enfeebs.MND) then
			equip(sets.midcast.Enfeebling.MND)
		end
		if fuzzyMatch(spell.name, enfeebs.INT) then
			equip(sets.midcast.Enfeebling.INT)
		end
		if fuzzyMatch(spell.name, enfeebs.effect) then
			equip(sets.midcast.Enfeebling.effect)
		end
		equip(sets.midcast.Enfeebling.duration)
		if state.immuno.value == 'true' then
			if fuzzyMatch(spell.name, enfeebs.immuno) then
				equip(sets.midcast.Enfeebling.immuno)
			end
		end	
	end
end
...
state.immuno = M{['description'] = 'immuno'}
...
function set_combat_weapon()
	...
	elseif state.WeaponMode.value == 'GreatKatana2' then
		state.CombatWeapon:set('GreatKatana2')
...
function set_combat_form()
	-- force grip with 
	if S{"GreatAxe", "GreatKatana", "GreatSword", "Polearm", "Scythe", "Staff"}:contains(state.WeaponMode.value) then
		if state.SubMode.value ~= 'TH' then 
			state.SubMode:set('Grip')
		end
	-- if grip mode and not 2h weapon, default to shield
	elseif state.SubMode.value == 'Grip' then
		add_to_chat(7, "using grip for 1 hand weapon defaulting to shield f10 to cycle SubMode")
...
function global_on_load()
	...
	send_command('bind f10 gs c cycle SubMode')
	send_command('bind f11 gs c cycle RWeaponMode')
	send_command('bind f12 gs c cycle OffenseMode')
	send_command('bind ^f12 gs c cycle CastingMode') --ctrl
	...
	if not ccRunning then
		add_to_chat(7, "f9 gs c cycle WeaponMode")
		add_to_chat(7, "f10 gs c cycle SubMode")
		add_to_chat(7, "f11 gs c cycle RWeaponMode")
		add_to_chat(7, "f12 gs c cycle OffenseMode")
		add_to_chat(7, "Cntl+f12 gs c cycle CastingMode")
	end	
end
...
-- must require the debug library first
